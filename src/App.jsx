...
const FALLBACK_SALES = [import { useState, useEffect } from "react";
...
const FALLBACK_ROSTER = [
  { store:"KSL", name:"LIEW WEN BIN",  role:"OM", work:18, off:4, asst:5, notes:"" },
  { store:"KSL", name:"SAW KAI BIN",   role:"JE", work:22, off:5, asst:0, notes:"" },
  { store:"KSL", name:"TEOH AI YUN",   role:"JE", work:22, off:5, asst:0, notes:"" },
  { store:"KSL", name:"LEE RUI SHENG", role:"JE", work:22, off:5, asst:0, notes:"" },
  { store:"KSL", name:"MARCUS GOH",    role:"SP", work:10, off:0, asst:0, notes:"3 HD" },
  { store:"BI",  name:"JOSEPH TAN",    role:"AM", work:17, off:4, asst:7, notes:"" },
  { store:"BI",  name:"AARON CHUNG",   role:"JE", work:0,  off:0, asst:0, notes:"RESIGNED" },
  { store:"BI",  name:"HOW YEN LING",  role:"JE", work:19, off:8, asst:0, notes:"" },
  { store:"BI",  name:"GOH SUET TENG", role:"JE", work:22, off:6, asst:0, notes:"" },
  { store:"BI",  name:"TAN YI THIM",   role:"SP", work:14, off:0, asst:0, notes:"" },
  { store:"BI",  name:"LEONG KAR PEE", role:"SP", work:15, off:0, asst:0, notes:"" },
  { store:"TEB", name:"Data pending",  role:"—",  work:0,  off:0, asst:0, notes:"Upload HQ Roster" },
  { store:"KOM", name:"Data pending",  role:"—",  work:0,  off:0, asst:0, notes:"Upload HQ Roster" },
  { store:"MA",  name:"Data pending",  role:"—",  work:0,  off:0, asst:0, notes:"Upload HQ Roster" },
];
...
function generateAlerts(sales, roster, reviews) {
  ...
  stores.forEach(s => {
    const activeStaff = roster.filter(r => r.store === s && r.work > 0 && !r.notes?.includes("RESIGNED"));
    const resigned = roster.filter(r => r.store === s && r.notes?.includes("RESIGNED"));
    const min = storeMinStaff[s];

    if (resigned.length > 0) {
      alerts.push({
        level: "URGENT", store: s,
        title: `${s} Staff Resigned — ${resigned.map(r => r.name).join(", ")}`,
        message: `${resigned.length} staff left. Store may fall below minimum headcount of ${min} active staff.`,
        recommendations: [
          "Post job opening immediately on Jobstreet / Facebook",
          "Ask existing staff to refer friends — offer RM200 referral bonus",
          `Cover shifts with ASST from nearby stores temporarily`,
          "Brief remaining staff on extra responsibilities short-term",
        ],
      });
    }
...
const storeColors = { KSL:"#C8A97E", BI:"#A8C5A0", TEB:"#B8A0C8", KOM:"#9FB8C8", MA:"#C9A0A0" };
const roleColors  = { OM:"#C8A97E", AM:"#b89060", JE:"#7a7a8a", SP:"#6a6a7a", "—":"#3a3530" };
const channelColors = { Shopee:"#EE4D2D", Lazada:"#0F146D", TikTok:"#69C9D0" };
      ...
      <div style={{ padding:"14px 18px", maxWidth:860 }}>
        ...
        {tab==="sales" && (
          <div>
            ...
            {sales.map(d=>{
              ...
              const statusColor = !pct ? "#3a3530" : pct>=100 ? "#A8C5A0" : pct>=85 ? "#C8A97E" : "#cc3333";
              return (
                <div key={d.store} style={{ background:"#1e1a16", border:`1px solid ${store===d.store?storeColors[d.store]+"44":"#2a2520"}`, borderRadius:3, padding:"14px 16px", marginBottom:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                    <div>
                      <div style={{ fontSize:9, color:storeColors[d.store], fontFamily:"monospace", letterSpacing:2, textTransform:"uppercase" }}>{d.store} — {storeNames[d.store]}</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:9, color:"#4a4038", fontFamily:"monospace" }}>TARGET</div>
                      <div style={{ fontSize:15, color:"#d4c8bc", fontFamily:"monospace", fontWeight:"bold" }}>RM {d.target.toLocaleString()}</div>
                    </div>
                  </div>
                  {d.actual ? (
                    <>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                        <span style={{ fontSize:10, color:"#6b5f52", fontFamily:"monospace" }}>RM {d.actual.toLocaleString()}</span>
                        <span style={{ fontSize:12, fontFamily:"monospace", fontWeight:"bold", color:statusColor }}>{pct}%</span>
                      </div>
                      <div style={{ height:6, background:"#2a2520", borderRadius:3 }}>
                        <div style={{ height:"100%", width:`${Math.min(pct,100)}%`, background:statusColor, borderRadius:3, transition:"width 0.4s" }} />
                      </div>
                      {pct < 85 && (
                        <div style={{ fontSize:10, color:"#c87040", marginTop:6, fontFamily:"monospace" }}>
                          ⚠ RM {(d.target-d.actual).toLocaleString()} behind — see Alerts tab for recommendations
                        </div>
                      )}
                    </>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ flex:1, height:6, background:"#2a2520", borderRadius:3 }} />
                      <span style={{ fontSize:9, color:"#3a3530", fontFamily:"monospace" }}>Upload Auto Count to populate</span>
                    </div>
                  )}
                </div>
              );
        ...
        {tab==="online" && (
          <div>
            ...
            {online.map(o=>{
              const pct = o.actual ? Math.round(o.actual/o.target*100) : null;
              const c = channelColors[o.channel] || "#C8A97E";
              return (
                <div key={o.channel} style={{ background:"#1e1a16", border:`1px solid #2a2520`, borderLeft:`3px solid ${c}`, borderRadius:3, padding:"14px 16px", marginBottom:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div style={{ background:"#141210", border:"1px solid #2a2520", borderRadius:3, padding:"14px 16px", marginTop:4 }}>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                ...
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:9, color:"#4a4038", fontFamily:"monospace" }}>Target: RM {totalOnlineTarget.toLocaleString()}</div>
                  {totalOnlineActual && <div style={{ fontSize:13, color:"#C8A97E", fontFamily:"monospace", fontWeight:"bold" }}>Actual: RM {totalOnlineActual.toLocaleString()}</div>}
                </div>
              </div>
            </div>
        ...
        {tab==="reviews" && (
          <div>
            <div style={{ fontSize:9, color:"#4a4038", fontFamily:"monospace", letterSpacing:2, textTransform:"uppercase", marginBottom:14 }}>
              Google Reviews — 4 Outlets — Auto-synced Daily
            </div>
            {[...reviews].sort((a,b)=>b.rating-a.rating).map((r,i)=>(
              <div key={i} style={{
                background: r.rating<4?"#1e1010":"#1e1a16",
